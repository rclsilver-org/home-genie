// Command hgenied is the home-genie server.
//
...
import (
	...
	"github.com/rclsilver-org/home-genie/server/internal/api"
	"github.com/rclsilver-org/home-genie/server/internal/cli"
	"github.com/rclsilver-org/home-genie/server/internal/config"
	...
	"github.com/rclsilver-org/home-genie/server/internal/reminder"
	"github.com/rclsilver-org/home-genie/server/internal/store"
	"github.com/rclsilver-org/home-genie/server/internal/version"
)

func main() {
	if err := dispatch(os.Args[1:]); err != nil {
		fmt.Fprintf(os.Stderr, "hgenied: %v\n", err)
		os.Exit(1)
	}
}
...
// runAdmin implements `hgenied admin create`.
func runAdmin(args []string) error {
	if len(args) == 0 || args[0] != "create" {
		return errors.New("usage: hgenied admin create -username <name> [-display-name <name>] [-config <file>]")
	}
...
func runServer(args []string) error {
	flags := flag.NewFlagSet("hgenied", flag.ContinueOnError)
	var (
		configPath  = flags.String("config", config.DefaultConfigFile(), "path to the configuration file")
	...
	if count, err := repository.CountUsers(); err == nil && count == 0 {
		logger.Warn("no account exists yet; create the break-glass one with " +
			"`hgenied admin create -username <name>`")
	}
